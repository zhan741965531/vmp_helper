...
def generate_dishi():
    ...
    for key,value in enumerate(fuzeren):
        value = value.replace("\r\n","")
        fuzeren[key] = value
    ...
    print("======================")
    print(yewuxitong)
    for key,value in enumerate(yewuxitong):
        value = value.replace("\r\n","")
        yewuxitong[key] = value
    ...
    old_data = xlrd.open_workbook("模板生成\地市现存中高危漏洞统计情况.xls")
    ...
    for each in range(the_length1):
        if each >= 1:
            ...
            mail = str(sheet1.cell_value(each,4)).replace("\r\n","")
            user = str(sheet1.cell_value(each,3)).replace("\r\n","")
    ...
    for index1,data1 in enumerate(list_old_system):
        ...
        for index2,data2 in enumerate(yewuxitong):
            if data1 == data2:
                ...
                print(data1,"==>",fuzeren[index2],"==>",list_old_mail[index1])
            else:
                pass
            """
            if list_old_user[index1] == fuzeren[index2]:
                print(list_old_user[index1],"==",fuzeren[index2],"==>",list_old_mail[index1])
                table2.write(index1+1,4,list_old_mail[index1],sytle2)
                print(index1+1,index1)
            else:
                pass
            """
    name = "地市现存中高危漏洞统计情况" + today + ".xls"
